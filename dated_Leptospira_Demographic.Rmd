...
```
### Write Isolates into a single file for separate analysis
```{r}

Brazil_172 <- edited_date_389 %>% subset(country == "Brazil")

# number of unique species in Brazil
unique(Brazil_172$species) # 15 including "unidentified sp."

# time interval
latest <- max(Brazil_172$Year) # 2017
earliest <- min(Brazil_172$Year) # 1950
latest-earliest

Brazil_172$Geographic.Location.x
...
***- "sp." has 39 isolates, however, some of them actually has a species name already***
    ***- find species names for "sp." in***
    ```Dropbox/5.Rachel-projects/Phylogeography/Lepto_assemblies_V2/ncbi+self_973.csv```
```{r}
unique(edited_date_389$species)
species_frequency <- ggplot(edited_date_389,aes(x=species)) +
  geom_bar() + 
