...
file <- as.data.frame(read.csv(paste0(file_path,"pirate_roary_pres_abs_accessory.csv")))
# head(file)
...
```{r}
inter_meta_2 <- read.csv("/Users/rachel/Dropbox/5.Rachel-projects/Phylogeography/SeroAll/all_pathogenic_sero_597.csv")
...
# pca analysis
pca_pa <- prcomp(pca_matrix[-c(1:12)]) # exclude metadata columns
...
ggplot(pca_matrix, aes(x=Serovar_2.x))+
  geom_bar()+
  theme_bw()+
...
pca_pa$x
pca_plot <- autoplot(pca_pa, data = pca_matrix, colour = 'Isolation.Country.x',x=2,y=3)+
  theme_bw() +
