...
ggplot(pca_matrix, aes(x=Serovar_2))+
  geom_bar()+
  theme_bw()+
...
current_genes %>% dunn_test(value ~ serogroups, p.adjust.method = "BH")  %>% subset(group1 == "Autumnalis" | group2 == "Autumnalis")
...
for(grp in serogroup){
  serogroup_value <- current_genes %>% subset(serogroup == grp) %>% select(value)
