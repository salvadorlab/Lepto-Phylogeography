...
# -v invert matching
# -p pattern
# cat $dir_path/snippy/clean.full.aln | seqkit grep -v -p Reference > $dir_path/snippy/no_ref_core.fasta 
...
# $dir_path/snippy/no_ref_core.fasta

# convert gubbins output gff file for recomination region's coordinates to bed file
# gff2bed < all_interrogans_gubbins_noref.recombination_predictions.gff > all_interrogans_gubbins_noref.recombination_predictions.bed
# cat all_interrogans_gubbins_noref.recombination_predictions_sorted.bed | uniq > all_interrogans_gubbins_noref.recombination_predictions.bed
...
# # download bed file, add the prophage regions coordinates detected to the bed file, change header of the bed file, ex. NC_004342.2
# # check the different headers for the two chromosomes, share the same coordinate scale, but need to label in the bed file differently.
# # # ex. chromosome one for GCF000092565.1 has length 4338763nts, need masked region smaller than 4338762nts need to label as NC_004342.2, after NC_004343.2 in the bed file
# # upload back to sapelo2
# # change header of the refernece genome (in .fna file, ex. NC_004342.2)
# # use bedtools to sort the bed file again
# sortBed -i all_interrogans_gubbins_noref.recombination_predictions.bed > all_interrogans_gubbins_noref.recombination_predictions_sorted.bed
# # take the unique regions only 
# cat all_interrogans_gubbins_noref.recombination_predictions_sorted.bed | uniq > all_interrogans_gubbins_noref.recombination_predictions.bed

# # mask the FASTA formatted mutliple sequence alignment file from recombination region and prophages with "X"
/home/rx32940/miniconda3/bin/snippy-core \
--mask "$dir_path/gubbins_noref/all_interrogans_gubbins_noref.recombination_predictions.bed" \
